...
namespace bob {
    ...
    [[nodiscard]] std::string hey(std::string_view message)
    {
        ...
        const auto trimmed_end   = std::find_if(std::crbegin(message), std::crend(message), [](const char c){ return !std::isspace(c); }).base();

        if (message.empty() || std::all_of(begin, trimmed_end, [](const char c){ return std::isspace(c); }))
            return "Fine. Be that way!";
        
        const bool is_question = *(trimmed_end - 1) == '?';
